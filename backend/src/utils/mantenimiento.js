...
// tiene que confirmarse a sí mismo.
async function pendientesDe(localId, excluirReportes = []) {
  ...
  `, [localId, excluirReportes.filter(Boolean)]);
  return rows;
}
...
async function faltantesMantenimiento(reporte) {
  ...
  const pendientes = await pendientesDe(reporte.local_id, [reporte.id]);
  return pendientes
