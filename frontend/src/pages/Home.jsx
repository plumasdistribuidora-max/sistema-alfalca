...
export default function Home() {
  ...
  useEffect(() => {
    ...
    function setContent(key) { ball.innerHTML = SVG[key]; }
    ...
    seq.forEach((step, i) => {
      ...
      timers.push(setTimeout(() => {
        ball.animate(
          [{ top: `${fromY}px` }, { top: `${FLOOR_Y}px` }],
          { duration: DROP_DUR, easing: 'cubic-bezier(0.45,0,0.7,1)', fill: 'forwards' }
        );
      }, t));
      t += DROP_DUR;
      ...
      timers.push(setTimeout(() => {
        squash();
        if (i < seq.length - 1) setContent(seq[i + 1].key);
      }, t));
    ...
    return () => {
      timers.forEach(entry => {
        if (entry && entry._iv !== undefined) clearInterval(entry._iv);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-6">
      <div className="ahg-anim-stage">
        <div className="ahg-floor" />
